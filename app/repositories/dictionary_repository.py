...
def get_cached_senses(session: Session, entry_id) -> list[DictionarySense]:
    statement = select(DictionarySense).where(DictionarySense.entry_id == entry_id)
    return session.exec(statement).all()


def get_cached_examples(session: Session, entry_id) -> list[DictionaryExample]:
    statement = select(DictionaryExample).where(DictionaryExample.entry_id == entry_id)
    return session.exec(statement).all()
